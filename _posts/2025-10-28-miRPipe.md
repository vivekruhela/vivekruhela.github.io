...
### How to interpret the benchmark responsibly

- It supports the performance of the tested 2022 implementations under the authors' simulation and real-data design.
- It does not establish permanent superiority over later releases or newly developed tools.
- Accuracy can depend on reference databases, parameter tuning, read simulation, class balance, library chemistry, species, and the definition of a true novel miRNA.
- The reverse-complement experiment remained challenging: the paper reports that miRPipe detected 374 of 887 reverse-complement high-confidence miRNAs. This was more than the comparators in that test, but it was not complete recovery.
- A current claim of superiority requires a new, version-locked benchmark using independent calibration and validation data.
...
- The published implementation is human-oriented.
- Candidate novel-miRNA labels are computational hypotheses until independently validated.
- Same-seed relationships are informative but do not prove identical biological function.
- piRNA annotation depends strongly on the database and mapping policy.
- The supplied differential-expression design handles only a simple treated-versus-untreated comparison.
- Real-data literature overlap is supportive evidence, not an unbiased gold standard.

### Current implementation limitations

- The repository does not contain a `miRPipe.py` command-line program; the supported artifact is `mirpipe_pipeline.ipynb`.
- The source-only installation instructions sometimes shown elsewhere are incomplete because the workflow requires many tools, references, and custom scripts beyond `requirements.txt`.
- Several dependencies and URLs are old or unpinned.
- DASHR browser automation depends on an older browser-driver stack and an external website.
- The Docker build is not pinned to an immutable base-image digest.
- There is no current automated test suite, release workflow, or continuous-integration check in the repository.
- The 24-nt boundary differs between the paper description and current splitting script.
- The tutorial refers to files/directories that are not present in the repository root, so the Docker image and GitHub checkout may not contain identical teaching assets.

These points do not erase the methodological contribution. They define the engineering work required for a reliable 2026 refresh.
...
- [ ] Record the repository commit SHA.
- [ ] Record the Docker image tag and digest.
- [ ] Record the host operating system, CPUs, RAM, and container runtime.
- [ ] Record the adapter and library-preparation protocol.
- [ ] Record genome build and all annotation-database versions.
- [ ] Save checksums for input FASTQ and reference files.
- [ ] Save the exact sample sheet used in the analysis.
- [ ] Validate sample order against count-matrix columns.
- [ ] Save FastQC/MultiQC and per-stage logs.
- [ ] Report read counts before and after every filter.
- [ ] Report parameters for Bowtie, miRDeep*, CD-HIT, Rfam, and DESeq2.
- [ ] Retain full count matrices and complete statistical results.
- [ ] Separate exploratory analysis from independent validation.
- [ ] Validate high-priority candidates experimentally when making biological claims.
...
This guide describes the published miRPipe method, cross-checks it against the current public repository, and flags discrepancies that must be resolved before a new production analysis. It is not a claim that the current repository has been revalidated end to end on modern operating systems.
